...
import { getTanamanKonversiData } from '../services/firestoreService';
...
export default {
  ...
  setup() {
    ...
    const calculateProgressTK = (item) => {
      // Aktivitas yang relevan untuk progress TK (sesuaikan dengan field di Firestore)
      const activities = [
        { rencana: item.membongkarTunggul, realisasi: item.membongkarTunggul },      
        { rencana: item.merumpukTunggul, realisasi: item.merumpukTunggul },     
        { rencana: item.ripping, realisasi: item.ripping },    
        { rencana: item.luku, realisasi: item.luku },   
        { rencana: item.pembuatanParit, realisasi: item.pembuatanParit },  
    ...
    const getRealizedArea = (item) => {
      const activities = [
        { realisasi: item.membongkarTunggul },   
        { realisasi: item.merumpukTunggul },  
    ...
    const fetchProgressDataToday = async () => {
      ...
      rawData.value.forEach(item => {
        ...
        const progressTK = calculateProgressTK(item);
      ...
      if (filterKebun.value && filterPaket.value) {
        ...
        rawData.value.forEach(item => {
          ...
          if (item.kebun === filterKebun.value && item.namaVendor === filterPaket.value) {
            // Hitung Progress TK dengan fungsi baru
            const lcPersentase = calculateProgressTK(item);
      ...
      rawData.value.forEach(item => {
        ...
        const activities = [
          { 
            ...
            rencana: item.membongkarTunggul, 
            realisasi: item.membongkarTunggul 
          },
          { 
            name: 'Merumpuk Tunggul', 
            rencana: item.merumpukTunggul, 
            realisasi: item.merumpukTunggul 
          },
          { 
            name: 'Ripping', 
    ...
    const processedRawData = computed(() => {
      ...
      // Convert Firestore data to format expected by DataGridTK
      const table = {
        rows: []
      };
      ...
      rawData.value.forEach((item, index) => {
        const row = {
          c: [
            ...
            { v: item.namaVendor },
            { v: item.luas },
            { v: item.membongkarTunggul },
            { v: 0 }, // Placeholder for hari ini
            { v: item.membongkarTunggul },
            { v: item.merumpukTunggul },
            { v: 0 }, // Placeholder for hari ini
            { v: item.merumpukTunggul },
            { v: item.ripping },
            { v: 0 }, // Placeholder for hari ini
    ...
    const refreshData = async () => {
      try {
        loading.value = true;
        rawData.value = await getTanamanKonversiData();
