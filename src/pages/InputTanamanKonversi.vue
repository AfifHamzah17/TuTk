<template>
  <div class="container">
    <h1 class="page-title">Input Data Tanaman Konversi</h1>
...
<script>
import { addTanamanKonversiData, getTanamanKonversiData } from '@/services/firestoreService';
import { formatDate } from '@/utils/dateUtils';

export default {
  name: 'InputTanamanKonversi',
  data() {
    return {
      formData: {
        kebun: '',
  ...
  methods: {
    ...
    async fetchData() {
      this.loading = true;
      try {
        const data = await getTanamanKonversiData();
        this.dataList = data;
      } catch (error) {
    ...
    async handleSubmit() {
      ...
      try {
        await addTanamanKonversiData(this.formData);
