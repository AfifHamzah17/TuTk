      ...
      <table class="data-table">
        <thead>
          <tr>
            ...
            <!-- Progress NPK -->
            <th class="w-32">Realisasi NPK SM I</th>
            
            <!-- Realisasi Dolomit -->
            <th class="w-32">Realisasi Dolomit SM I</th>
            
            <!-- Sisa Pemupukan SM I -->
            <th class="w-64">Sisa Pemupukan SM I</th>
        ...
        <tbody class="data-grid-body">
          <template v-for="(group, groupName) in paginatedGroupedData" :key="groupName">
            ...
            <tr class="distrik-group">
              <td colspan="14" class="distrik-header px-6 py-3 cursor-pointer" @click="toggleGroup(groupName)">
              ...
              <tr v-for="(row, index) in group" :key="index" class="data-grid-row">
                ...
                <td class="data-grid-cell">
                  <ProgressItem
                    title="Dolomit SM I"
                    unit="Ton"
                    :rencana="row.rencanaDolomit"
                    :hariIni="row.realisasiHariIniDolomit"
                    :sdHariIni="row.realisasiSdHariIniDolomit"
                    :persentase="row.progressDolomit"
                  />
                </td>
  ...
  computed: {
    processedData() {
      ...
      for (let i = 0; i < rows.length; i++) {
        ...
        const realisasiHariIniNPK = cells[8] ? parseFloat(cells[8].v) || 0 : 0;
        const realisasiSdHariIniNPK = cells[9] ? parseFloat(cells[9].v) || 0 : 0;
        
        // Data Dolomit [11], [12], [13]
        const rencanaDolomit = cells[11] ? parseFloat(cells[11].v) || 0 : 0;
        const realisasiHariIniDolomit = cells[12] ? parseFloat(cells[12].v) || 0 : 0;
        const realisasiSdHariIniDolomit = cells[13] ? parseFloat(cells[13].v) || 0 : 0;
        
        // Data Stok Pupuk 
        const stokNPK = cells[15] ? parseFloat(cells[15].v) || 0 : 0;
        const stokDolomit = cells[16] ? parseFloat(cells[16].v) || 0 : 0;

         // Data SPPBJ 
        const sppbjNPK = cells[18] ? (cells[18].f || cells[18].v) : '';
        const sppbjDolomit = cells[19] ? (cells[19].f || cells[19].v) : '';

        // Data Jangka Waktu 
        const jangkaWaktuNPK = cells[20] ? (cells[20].f || cells[20].v) : '';
        const jangkaWaktuDolomit = cells[21] ? (cells[21].f || cells[21].v) : '';

         // Data Persentase Dropping 
        const persentaseDroppingNPK = cells[22] ? parseFloat(cells[22].v) || 0 : 0;
        const persentaseDroppingDolomit = cells[23] ? parseFloat(cells[23].v) || 0 : 0;
        
        // Hitung sisa pemupukan
        const sisaNPK = rencanaNPK - realisasiSdHariIniNPK;
        const sisaDolomit = rencanaDolomit - realisasiSdHariIniDolomit;
        
        // Hitung progress
        const progressNPK = calculatePercentage(realisasiSdHariIniNPK, rencanaNPK);
        const progressDolomit = calculatePercentage(realisasiSdHariIniDolomit, rencanaDolomit);
        ...
        result.push({
          ...
          rencanaDolomit: rencanaDolomit,
          stokNPK: stokNPK,
          stokDolomit: stokDolomit,
          realisasiHariIniNPK: realisasiHariIniNPK,
          realisasiHariIniDolomit: realisasiHariIniDolomit,
          realisasiSdHariIniNPK: realisasiSdHariIniNPK,
          realisasiSdHariIniDolomit: realisasiSdHariIniDolomit,
          sisaNPK: sisaNPK,
        ...
        console.log(`Data ditambahkan dari baris ${i}:`, {
          ...
          luas_ha: luas_ha,
          sppbjNPK: sppbjNPK,
          sppbjDolomit: sppbjDolomit,
          jangkaWaktuNPK: jangkaWaktuNPK,
          jangkaWaktuDolomit: jangkaWaktuDolomit,
          durasiJangkaWaktuNPK: durasiJangkaWaktuNPK,
          durasiJangkaWaktuDolomit: durasiJangkaWaktuDolomit,
          persentaseDroppingNPK: persentaseDroppingNPK,
          persentaseDroppingDolomit: persentaseDroppingDolomit,
